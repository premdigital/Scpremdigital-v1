...
# Fix CLI menu output using regex to catch any variations
old_cli_pattern = r'echo -e "\\n\$\{Y\}Akun Berhasil Dibuat!\$\{NC\}".*?echo -e "Host     : \$?\$\(cat /etc/vps-domain\.txt\)"'
...
content = re.sub(old_cli_pattern, new_cli, content, flags=re.DOTALL)
